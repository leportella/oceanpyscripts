...
from ncwork import find_index_of_nearest_xy
...
for k in range(1,4):
    ...
    print 'Dados de ST00{} lidos...'.format(k)
    
    #calcula o tempo
    modelo[k]['tempo']=[]
    initdate=datetime(2011,1,1,0,0,0)
    for t in otime:
        modelo[k]['tempo'].append(np.add(initdate, timedelta(seconds=int(t))))
    
    modelo[k]['tempo_local']=np.subtract(modelo[k]['tempo'], timedelta(hours=3))
    ...
    plt.close()
    print 'Dados de ST00{} prontos'.format(k)
    #k=1
    #plt.figure(figsize=(15,5))
